...
export default function UpdateForm({ error, setError, updateValues }) {
  ...
  const updateSubmit = async (values) => {
    ...
    if (
      updateValues.detail_type_user.some(
  ...
  const {
    ...
    isChange,
  } = useSignUp(imgData, updateSubmit, updateValues, setError);
      ...
      Tạo tài khoản
        ...
        >
          ...
          >
                  ...
                  return (
                    <div key={item.type_user_id} className="ml-5 mt-5">
                      ...
                      <GreenSwitch
                        label="Admin"
                        onChange={() =>
                          handleUpdatePermission(item?.type_user_id)
                        }
            ...
            {error?.permission ? (
              <p className="error-text text-2xl">
                Vui lòng cấp quyền cho tài khoản
              </p>
            ) : (
              ""
            )}
